...
class PurchaseController extends HomeController
{
    ...
    public function get_purchases() {
        $purchases = Purchase::leftJoin('lb_Alternatives as a', 'Purchases.AlternativeID', '=', 'a.id')->leftJoin('Orders as o', 'a.OrderID', '=', 'o.id')->leftJoin('lb_status as s', 'o.situation_id', '=', 's.id')->leftJoin('users', 'o.MainPerson', '=', 'users.id')->leftJoin('Departments as d', 'users.DepartmentID', '=', 'd.id')->leftJoin('accounts', 'Purchases.account_id', '=', 'accounts.id')->leftJoin('companies', 'accounts.company_id', '=', 'companies.id')->leftJoin('lb_units_list as u', 'a.unit_id', '=', 'u.id')->where(['Purchases.deleted'=>0, 'a.deleted'=>0 ,'o.deleted'=>0, 'accounts.deleted'=>0])->orderBy('Purchases.id', 'DESC ')->select('Purchases.id as id', 'o.Product', 'a.Brend', 'a.Model', 'a.cost', 'a.total_cost', 'a.pcs', 'u.Unit', 'Purchases.account_id', 'Purchases.Remark', 'o.deadline', 'Purchases.created_at', 'users.name', 'users.surname', 'd.Department', 'accounts.account_no', 'accounts.account_doc', 'accounts.created_at as account_date', 'accounts.lawyer_doc', 'accounts.lawyer_confirm_at', 'companies.name as company', 'companies.address', 'companies.phone', 'Purchases.qaime_no', 'Purchases.qaime_doc', 'Purchases.qaime_date', 's.status', 's.color')->paginate(30);
