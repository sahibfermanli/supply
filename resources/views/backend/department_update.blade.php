                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            <form id="form" data-parsley-validate class="form-horizontal form-label-left" method="post" action="">
                                {{csrf_field()}}
                                {{Form::bsTextRequired('Department', 'Department', $department->Department)}}
                                {{Form::bsTextRequired('Company', 'Company', $department->Company)}}
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select class="form-control col-md-7 col-xs-12" name="authority_id" id="authority_id" required>
                                            @foreach($authorities as $authority)
                                                @if($department->authority_id == $authority->id)
                                                    <option selected value="{{$authority->id}}">{{$authority->title}}</option>
                                                @else
