                        ...
                        <div class="accounts-mobile">
                            <ul class="nav child_menu show-accounts">
                                @foreach($accounts as $account)
                                    <li class="account-li" title="{{$account->account_no}}"><a style="color: black !important;" class="account-select" href="/law/pending/orders?account_id={{$account->id}}">{{$account->company}}</a></li>
                                @endforeach
                            </ul>
                        </div>
                        <div class="x_content">
                                ...
                                <div style="display: {{$table_display}};">
                                    <div style="display: inline-block; float: left;">
                                        <form action="" method="post" enctype="multipart/form-data">
                                            ...
                                            {{csrf_field()}}
                                            {{--<input type="file" name="file" class="form-control" style="width: 30%; display: inline-block;">--}}
                                    ...
                                    <a style="display: inline-block; float: right;" class="btn btn-success" title="{{$orders[0]->account_date}}" target="_blank" href="{{$orders[0]->account_doc}}">Hesab: <i class="fa fa-download"></i> {{$orders[0]->account_no}}</a>
                                    <table class="table table-bordered">
                                        ...
                                        @foreach($orders as $order)
                                            ...
                                            <tr class="even pointer" id="row_{{$row}}">
                                                ...
                                                <td style="min-width: 200px;">{{$order->name}} {{$order->surname}} , {{$order->Department}}</td>
                                                <td>{{$order->Product}}</td>
                                                <td>{{$order->Brend}}</td>
                                                <td>{{$order->Model}}</td>
                                                <td>{{$order->pcs}}</td>
                                                ...
                                                <td>
                                                    @if(!empty($order->qaime_doc))
                                                        <a class="btn btn-success btn-xs" title="{{$order->qaime_date}}" target="_blank" href="{{$order->qaime_doc}}"><i class="fa fa-download"></i> {{$order->qaime_no}}</a>
                                                    @else
                                                ...
                                                <td style="min-width: 78px;">
                                                    <center>
                                                        @if(!empty($order->image))
                                                            <span title="Şəkli göstər" onclick="get_data('{{$order->order_id}}', 'show_image');" class="btn btn-success btn-xs data-modal"><i class="fa fa-image"></i></span>
                                                        @else
                                                            <span title="Şəkil yoxdur" disabled="true" class="btn btn-success"><i class="fa fa-image"></i></span>
                                                        @endif

                                                        @if(!empty($order->deffect_doc))
                                                            <a style="background-color: #0e90d2;" title="Qüsur aktını endir endir" href="{{$order->deffect_doc}}" class="btn btn-success btn-xs" target="_blank"><i class="fa fa-file"></i></a>
                                                        @else
